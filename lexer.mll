...
and genre perso r = parse
    ...
    |"Plateforme" espaces '\n' 
        {let (pos,speed,maxSpeed,pv,(g,m,d,s), y) = (pos perso r lexbuf ) in ((Objet.Plateforme),pos, speed, maxSpeed, pv, (Anim.create g m d s r),  y)} 
    |"Ennemi\n"
        {let (pos,speed,maxSpeed,pv,(g,m,d,s), y) = (pos perso r lexbuf ) in ((Objet.Ennemi),pos, speed, maxSpeed, pv,  (Anim.create g m d s r), y)}
    |"Wall\n" 
        {let (pos,speed,maxSpeed,pv,(g,m,d,s), y) = (pos perso r lexbuf ) in ((Objet.Wall),pos, speed, maxSpeed, pv,  (Anim.create g m d s r),  y)}
    |"Door" espaces (nomFichierTXT as t) '\n' 
        {let (pos,speed,maxSpeed,pv,(g,m,d,s), y) = (pos perso r lexbuf ) in ((Objet.Door t ),pos, speed, maxSpeed, pv,  (Anim.create g m d s r), y )} 
    |"Projectile\n" 
        {let (pos,speed,maxSpeed,pv,(g,m,d,s), y) = (pos perso r lexbuf ) in ((Objet.Projectile),pos, speed, maxSpeed, pv,  (Anim.create g m d s r), y )}
    |_ as c {Printf.printf "Erreur : %c" c;raise Erreur_de_syntaxe}
    |eof {raise Erreur_de_syntaxe}
	
and pos2 perso r = parse
    |"Position" espaces (coupleDigit ) '\n'
        {let y = (entities perso r lexbuf ) in ((int_of_string x1,int_of_string x2),y)}
    |_ as c {Printf.printf "Erreur : %c" c;raise Erreur_de_syntaxe}
    |eof {raise Erreur_de_syntaxe}
