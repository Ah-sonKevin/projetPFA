...
module GameMap : GameMap  = struct
  ...
  let drawMini scene window render sizeMap =
    ...
    drawRect (windowSizeX,0) (sw1Int,sw2Int) render;
    let ratioX = sw1Float /. (float_of_int demiWidth) in
    let ratioY = sw2Float /. (float_of_int demiHeight) in
    ...
    List.iter (fun x ->
      ...
      in
      let (yPers) = 
        if yPer >demiHeight - sw2Int/2 then 
          (demiHeight - 210/2)
        else if yPer < sw2Int/2 then sw2Int/2 else yPer
      in
      let (rx,ry) = ((float_of_int (objX+(-xPers+sw1Int/2)))*.ratioX, ((float_of_int (objY+(-yPers+sw2Int/2)))*.ratioY)) in
      if (((rx+.(float_of_int objWidth))<=float_of_int sw1Int)&&(ry+.(float_of_int objHeight)<= float_of_int sw2Int)) then      
